...
export const session = reatomObservable(
  {
    getState: () => betterAuthSession.get() as AuthSessionSnapshot,
    subscribe: (listener) =>
      betterAuthSession.subscribe((sessionSnapshot) => {
        listener(sessionSnapshot as AuthSessionSnapshot)
      }),
  },
  'session',
)
...
export const isSessionPending = computed(() => {
  ...
  return authSession.isPending && !authSession.isRefetching
}, 'isSessionPending')
